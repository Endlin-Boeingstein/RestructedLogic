...
bool OBBHashEquals() {
    ...
    rsb_self_path_str = rsb_path_str + "/" + ori_rsb_name;
    //apk内数据包哈希值
    apkOBBHash = HashComparer::get_asset_hash_64(apk, "assets/" + ori_rsb_name);
    if (apkOBBHash == 0) {
        LOGI("哈希校验：非直装包");
        return true;
    }
    if (check_magic_number(rsb_self_path_str.c_str(), "EBRL")) {
        ...
        OBBHash = HashComparer::read_hash_after_header_64(rsb_self_path_str.c_str());
    }
    else {
        //如果大小不一，直接就是不一样
        ...
        OBBHash = HashComparer::compute_file_hash(fs::path(rsb_self_path_str));
    }
    bool result = HashComparer::are_hashes_identical_64(apkOBBHash, OBBHash);
    LOGI("Hash End");
    return result;
}
...
int hkLawnAppScreenWidthHeight(float* a1, int a2) {
    ...
    mOrigScreenWidth = iPtr[1860];
    mOrigScreenHeight = iPtr[1864];
